...
Beta Trees
```{r}
p <- ggtree(sim_beta$phy_ml) + theme_tree2() + geom_tiplab(size=1.0)+  scale_x_ggtree()
pdf("../manuscript_figs/kmb_input.pdf", 8, 8)
p
dev.off()
p
```
Modified Beta Trees
```{r}
p <- ggtree(sim_kmb$phy_ml) + theme_tree2() + geom_tiplab(size=1.0)+  scale_x_ggtree()
pdf("../manuscript_figs/kmb_input.pdf", 8, 8)
p
dev.off()
p
...
side_by_side <- function(phy_gt, res)
{
    ...
    plt_ci <- plot_treeCI(res, mrsd = "01-01-2020") + ggtitle("Estimated Credible Tree") 
    d <- ggplot_build(plt_ci)
    
    a <- d$plot$scales$get_scales("colour")$range$range[[2]]
    a <- max(max(msize_outgoing, na.rm=T),a)

    plt_ci <- plt_ci + scale_colour_viridis_b(breaks=2:max(a, na.rm=T)) + #(plt_ci$theme %+replace% theme(legend.position="none")) + 
        labs(color="Merger Size")

    plt_gt <- ggtree(phy_gt.tbl, mrsd = "01-01-2020", aes(color = msize)) + 
        scale_colour_viridis_b(breaks=2:max(a, na.rm=T)) +
...
```
```{r, eval=T}
p <- side_by_side(sim_beta$phy_gt, res_beta)
pdf("../manuscript_figs/beta_res.pdf", 8, 8)
p
dev.off()
p
```
...
```{r, eval=T}
p <- side_by_side(sim_kmb$phy_gt, res_kmb)
pdf("../manuscript_figs/kmb_res.pdf", 8, 8)
p
dev.off()
p
```
```{r,eval=T}
p<-ggtree(sample_timetree(res_beta,9)) + theme_tree2() + facet_wrap(~.id, ncol=3)
...
```{r,eval=T}
p<-ggtree(sample_timetree(res_kmb,9)) + theme_tree2() + facet_wrap(~.id, ncol=3)
pdf("../manuscript_figs/kmb_ps.pdf", 8, 8)
p
dev.off()
p
```
...
pdf("../manuscript_figs/beta_traces.pdf", 8, 8)
p
dev.off()
p
```
...
pdf("../manuscript_figs/kmb_traces.pdf", 8, 8)
p
dev.off()
p
```
...
pdf("../manuscript_figs/beta_pars.pdf", 8, 8)
p
dev.off()
p
```
...
```{r}
p<-plot_pars(res_kmb)
pdf("../manuscript_figs/kmb_pars.pdf", 8, 8)
p
dev.off()
p
```