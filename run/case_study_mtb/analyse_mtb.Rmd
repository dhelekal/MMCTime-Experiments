...
df_beta <- foo(res_beta,c("alpha","nu","mu","sigma"))
df_kmb <- foo(res_kmb,c("phi","alpha","nu","mu","sigma"))
df_kingman <- foo(res_kingman,c("nu","mu","sigma"))
```

Plot clock parameters
```{r,out.width="100%"}
mut_df <- rbind(cbind(df_beta[,c("mu", "sigma", "dataset")], model="Beta"),
    cbind(df_kmb[,c("mu", "sigma", "dataset")], model="Extended beta"),
    cbind(df_kingman[,c("mu", "sigma", "dataset")], model="Kingman"))
mDf <- melt(mut_df, measure.vars=c("mu","sigma"))
