...
```
Plot a summary tree for Extended beta
```{r, out.width="100%",eval=T}
p <- plot_densiCI(res_kmb, n_samp=100, mrsd=max(dates),layout="rectangular")
ggsave("../manuscript_figs/vibrio_densi_mm.png", plot=p, device = png, type="cairo", width=6, height=6, dpi=1000)
p
```
Plot a summary tree for kingman
```{r, out.width="100%",eval=T}
p <- plot_densiCI(res_km, n_samp=100, mrsd=max(dates),layout="rectangular")
ggsave("../manuscript_figs/vibrio_densi_km.png", plot=p, device = png, type="cairo", width=6, height=6, dpi=1000)
p
```
...
tmp1 <- cbind(res_kmb$draws[,c("mu", "sigma", "t_412")],"Extended Beta")
colnames(tmp1) <- c("mu", "sigma", "height", "prior")
tmp2 <- cbind(res_km$draws[,c("mu", "sigma", "t_412")],"Kingman")
colnames(tmp2) <- c("mu", "sigma", "height", "prior")
