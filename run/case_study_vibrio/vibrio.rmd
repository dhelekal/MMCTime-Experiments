...
```{r, eval=run_mcmc}
res_kmb <- mmctime(tree, dates, n_draws=1e3L, thin=2e3L, n_chain=4, model="km_beta", verbose=F, fix_root=T)
saveRDS(res_kmb, "res_kmb.rds")
res_km <- mmctime(tree, dates, n_draws=1e3L, thin=2e3L, n_chain=4, model="kingman", verbose=F, fix_root=T)
